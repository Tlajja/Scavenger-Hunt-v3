...
export default function CreateChallenge() {
    ...
    async function handleCreateTask(e) {
        ...
        try {
            ...
            if (taskDeadlineMode === 'absolute') {
                ...
                if (!iso) {
                    setError('Please pick a deadline date/time or choose another mode')
                    return
                }
                ...
                if (mins < 0 || hrs < 0) {
                    setError('Timer values must be non-negative')
                    return
                }

                const totalSeconds = hrs * 3600 + mins * 60
                if (totalSeconds <= 0) {
                    setError('Provide at least 1 minute or hour for the timer')
                    return
                }
            ...
            if (!res.ok) {
                setError(res.data?.message || res.data?.error || res.text || 'Failed to create task')
                return
            }
    ...
    const selectedTasks = selectedTaskIds
        .map(id => tasks.find(t => String(t.id ?? t.Id) === id))
        .filter(Boolean)
            ...
            <div className="card" style={{ maxWidth: 700, width: '100%' }}>
                ...
                <form onSubmit={handleSubmit}>
                            ...
                            >
                                {selectedTasks.map(t => (
                                    ...
                                    >
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => removeSelectedTask(String(t.id ?? t.Id))}
                                            disabled={creating}
                                            style={{
                    ...
                    {showCreateTask && (
                        ...
                        >
                            <h3 style={{ color: 'white', marginBottom: 16, fontSize: 18 }}>
                                Create Task
                            </h3>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                                    ...
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                        <input
                                            ...
                                            name="dlmode"
                                            value="timer"
                                            checked={taskDeadlineMode === 'timer'}
                                    ...
                                    >
                                        <div>
                                            ...
                                            <input
                                                ...
                                                min="0"
                                                step="1"
                                                value={taskTimerHours}
                                                onChange={e => setTaskTimerHours(e.target.value)}
                                                disabled={creatingTask}
                                            />
                                        </div>
                                        ...
                                        >
                                            Countdown starts when the challenge is created
                                        </div>
                                    </div>
                                )}
                            </div>
                    ...
                    <button
                        type="submit"
                        disabled={creating || !challengeName.trim() || !selectedTaskIds.length}
